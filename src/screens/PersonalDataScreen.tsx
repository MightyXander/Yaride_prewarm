...
import GenderSelect, { type GenderValue } from '../components/ui/GenderSelect';
import { AuthField } from '../components/AuthKit';
...
/**
 * PersonalDataScreen (issue #462) — «Личные данные» отдельным экраном.
 * Открывается строкой-переходом из Профиля (как «Мои машины»): flow-экран с
 * Header/BackButton и видимым навбаром, а сами 6 полей + форма заявки грузятся
 * уже здесь (на экране профиля подгрузов не видно). Логика целиком перенесена
 * из прежнего компонента PersonalDataSection (issue #456).
 *
 ...
 * плашка «на одобрении» с перечнем запрошенных изменений.
 */
      ...
      >
        {loading || !personal ? (
          loadError ? (
            ...
            // Скелет пока данные грузятся (подгруз происходит на своей странице).
            <div data-testid="personal-skeleton" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {FIELD_ORDER.map((field) => (
                <div key={field} style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <Skeleton h={12} w="30%" r={6} />
