...
## GitHub Issue #25: feat(ui): вечер 21–24 + постоянный водитель (бледно-жёлтая карточка)

**URL:** https://github.com/MightyXander/Yaride_prewarm/issues/25
**Метки:** enhancement, status: todo
**Ветка:** `feature/issue-25`

## Описание Issue

Экраны по SPEC §4 (вечерний коридор Центр→Брагино, 17:30–19:00):
- **21 — Главный (вечер):** вариант MainScreen с вечерними данными/окном.
- **22 — Карточка (вечер):** детали вечерней поездки.
- **23 — Водитель публикует (вечер):** вариант DriverPublish.
- **24 — «Домой как вчера»:** карточка постоянного водителя — БЛЕДНО-ЖЁЛТЫЙ фон (бледнее hero, по SPEC §3), быстрый повтор вчерашней брони.
## Общие требования
- Стек: React+Vite, inline-стили + src/index.css, БЕЗ Tailwind/HeroUI. Переиспользовать дизайн-систему src/components/ui/ и НОВЫЙ src/components/Header.tsx (из #21), единый мягкий фокус и «живые» кнопки (паттерны из #21), FloatingNav-навигацию.
- Расширить useNavigation + types для новых экранов; BackButton (Telegram+фолбэк) и scroll. Без мёртвых кнопок, тач-цели ≥44pt.
- Рыба-данные (по плану B рыбу убираем позже). Без платежей. Светлая+тёмная, 360/390px без горизонтального скролла.
- НЕ трогать backend/server.js/Dockerfile. oxlint/tsc -b/npm run build — зелёные. Источник визуала — мокапы design/ + SPEC §4.


## Комментарии к Issue

**@MightyXander** (2026-06-25T22:09:22Z):
## Director: уточнения к приёмке (запускается ПОСЛЕ мержа #24)

Экраны 21/22/23/24 (вечер, коридор Центр→Брагино 17:30–19:00). Источник — `design/mockup-light.html`/`mockup-dark.html` (подписи `.cap` 21–24).

Переиспользовать существующие экраны через параметризацию (НЕ дублировать вёрстку):
- **21 «Вечер · главный»** = `MainScreen` с доп. пропсами `title` (Topbar «Центр → Брагино») и `heroKicker` («Сегодня домой»). Вечерние рыба-trips в `App.tsx`. Тап → `trip-details` (вечерняя поездка); «Возьму попутчиков» → 23; «Ищу, кто подвезёт» → `empty-state`.
- **22 «Вечер · карточка»** = переиспользовать `TripDetailsScreen` с вечерним `Trip` (Марина, VW Polo синяя, Центр→Брагино, 17:40, ≈70). Отдельный экран НЕ нужен.
- **23 «Вечер · водитель публикует»** = `DriverPublishScreen` с доп. пропсами: `title`(«Я за рулём · домой»), маршрут обратный, времена `[17:30,17:40,18:00,18:30,другое]`, точка сбора «пл. Волкова, у фонтана». «Опубликовать поездку» → `booking-confirmed` (kind=publish).
- **24 «Домой как вчера»** = НОВЫЙ экран `HabitHomeScreen`: `Hero` (вечер) + карточка постоянного водителя с **БЛЕДНО-ЖЁЛТЫМ фоном** (бледнее hero, по SPEC §3 — класс `.regular` в мокапе). «Поехать домой как вчера» → `booking-confirmed`(kind=booking, вечерняя поездка); «Другие поездки на сегодня» → 21.

Точка входа: `ProfileScreen` — пункт «Вечер: домой как вчера» → 24. Новые `Screen`: `evening-main`(21), `evening-publish`(23), `habit-home`(24). Расширить types/useNavigation/App/FloatingNav. БЕЗ мёртвых кнопок, light+dark, 360/390 без горизонт. скролла, тач ≥44pt, haptics уместно. НЕ трогать backend. НЕ исправлять падежи. oxlint/tsc -b/npm run build — зелёные.
...
Реализуй вечерние экраны 21/22/23/24 СТРОГО по issue #25 и моему Director-комментарию (прочитай через gh issue view 25 --comments). Источник визуала — design/mockup-light.html и design/mockup-dark.html (подписи .cap 21–24). КЛЮЧЕВОЕ — переиспользуй существующие экраны через параметризацию, НЕ дублируй вёрстку: 21=MainScreen с новыми пропсами title+heroKicker (вечерние рыба-trips в App.tsx); 22=переиспользовать TripDetailsScreen с вечерним Trip (отдельный экран не нужен); 23=DriverPublishScreen с пропсами title/маршрут/времена[17:30,17:40,18:00,18:30,другое]/точка сбора; 24=НОВЫЙ HabitHomeScreen с Hero(вечер)+карточкой постоянного водителя БЛЕДНО-ЖЁЛТЫЙ фон (бледнее hero). Точка входа: ProfileScreen пункт «Вечер: домой как вчера» → 24. Новые Screen: evening-main(21), evening-publish(23), habit-home(24); расширь types/useNavigation/App/FloatingNav. ВАЖНО: кнопка «назад» уже глобальная (BackButton показывается на всех кроме intro/main/main-more) — не дублируй; на безхедерных экранах добавляй верхний клиренс ~52px под кнопку (как сделано в AlertPushScreen). Инлайн-стили+токены, БЕЗ Tailwind. Light+dark, 360/390 без горизонт. скролла, тач ≥44pt, haptics уместно, БЕЗ мёртвых кнопок. НЕ трогай backend/server.js/Dockerfile. НЕ исправляй русские падежи/грамматику. Перед коммитом oxlint/tsc -b/npm run build зелёные. Коммить в feature/issue-25, открой PR в main.

---

## Чеклист завершения

1. Реализуй задачу в ветке `feature/issue-25`
...
**IMPORTANT:** Целевой репозиторий: `MightyXander/Yaride_prewarm`. Все gh_manager-вызовы ОБЯЗАТЕЛЬНО с флагом `--repo MightyXander/Yaride_prewarm`. Env: `GH_REPO=MightyXander/Yaride_prewarm`.
