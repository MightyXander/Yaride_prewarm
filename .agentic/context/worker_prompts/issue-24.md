...
## GitHub Issue #24: feat(ui): сценарии пассажира — оценка, заявка, алерт-пуш, мои поездки, заявка опубликована (11,12,14,17,20)

**URL:** https://github.com/MightyXander/Yaride_prewarm/issues/24
**Метки:** enhancement, status: todo
**Ветка:** `feature/issue-24`

## Описание Issue

Экраны по SPEC §4:
- **11 — Оценка после поездки:** звёзды + быстрые теги + коммент.
- **12 — Заявка пассажира:** форма «нужно к HH:MM туда-то».
- **14 — Алерт-пуш:** карточка появившейся подходящей поездки (из подписки).
- **17 — Мои поездки:** список (имя/время/маршрут, предстоящие/прошлые).
- **20 — Заявка опубликована:** успех-стейт заявки.
## Общие требования
...
**@MightyXander** (2026-06-25T22:09:01Z):
## Director: уточнения к приёмке (делает этот же воркер)

Кросс-сквозные требования (помимо экранов 11/12/14/17/20):

1. **Кнопка «назад» слева сверху на ВСЕХ экранах, кроме `intro`/`main`/`main-more`.** Сейчас фоллбэк-кнопка в `src/components/BackButton.tsx` подавляется, когда существует `window.Telegram.WebApp` (а стаб `telegram-web-app.js` есть и в браузере) → в превью кнопки нет. Нужно: in-app кнопку рендерить ВСЕГДА при `show=true`; нативную Telegram BackButton вести только если `tg.isVersionAtLeast('6.1')` (добавить тип в `telegram.d.ts`). В `App.tsx` заменить `showBackButton = currentScreen !== 'intro'` на исключение `['intro','main','main-more']`.

2. **Фикс горизонтального скролла при переходах.** Slide-анимация экранов (`framer-motion`, x:±28) вызывает горизонтальный скролл. Добавить `overflowX: 'clip'` на контейнер с `maxWidth:'390px'` в `App.tsx`.

3. **Маршрутизация и точки входа (БЕЗ мёртвых кнопок).** Новые `Screen`: `passenger-request`(12), `request-published`(20), `alert-push`(14), `my-trips`(17), `rate-trip`(11). Расширить `types/navigation.ts`, `useNavigation` (PARENT_SCREEN + scrollPositions), роутинг в `App.tsx`, `FloatingNav.HIDDEN_ON`. Связать:
   - `EmptyStateScreen` (сейчас ActionCard БЕЗ onClick — мёртвые): «Оставить заявку» → 12; «Сообщить, когда появится» → 14.
   - 12 «Опубликовать заявку» → 20; 20 «Изменить заявку» → 12, «Отменить» → goBack.
   - 14 «Открыть и забронировать» → `trip-details` (поездка Марина) → существующий booking-flow.
   - `ProfileScreen`: добавить пункт меню «Мои поездки» → 17.
   - 17 «Создать поездку» → `driver-publish`; тап по прошлой поездке → 11; 11 close/«Отправить оценку» → goBack.

Источник визуала — `design/mockup-light.html` / `mockup-dark.html` (экран N в подписи `.cap`). Переиспользовать `src/components/ui/*`, `Header`, `lib/haptics`. Светлая+тёмная, 360/390 без горизонт. скролла, тач ≥44pt. НЕ трогать backend/server.js/Dockerfile. НЕ исправлять падежи/грамматику (текст дословно из мокапа). `oxlint`/`tsc -b`/`npm run build` — зелёные.
...
Реализуй экраны 11/12/14/17/20 пассажирских сценариев СТРОГО по telу issue #24 и по моему Director-комментарию в issue (прочитай оба через gh issue view 24 --comments). Источник визуала — design/mockup-light.html и design/mockup-dark.html (экран по подписи .cap). ОБЯЗАТЕЛЬНО выполни 3 кросс-сквозных пункта из Director-комментария: (1) BackButton слева сверху на всех экранах кроме intro/main/main-more (in-app кнопку рендерить всегда при show=true; нативную вести только при tg.isVersionAtLeast('6.1')); (2) overflowX:'clip' на контейнере maxWidth:390 в App.tsx (фикс горизонтального скролла при slide-переходах); (3) маршрутизация + точки входа без мёртвых кнопок (EmptyState/Profile/MyTrips и т.д.). Переиспользуй src/components/ui/*, Header, lib/haptics; инлайн-стили + CSS-токены, БЕЗ Tailwind. Light+dark, 360/390 без горизонт. скролла, тач ≥44pt. НЕ трогай backend/server.js/Dockerfile. НЕ исправляй русские падежи/грамматику (текст дословно из мокапа). Перед коммитом: oxlint, tsc -b, npm run build — все зелёные. Коммить в ветку feature/issue-24 и открой PR в main.

---

## Чеклист завершения

1. Реализуй задачу в ветке `feature/issue-24`
